...
------------------------------------------------------------------------------
  PASS Q01  Which region brings in the most revenue?
          2.7s | cols=Y | exec=Y | acc=Y | fmt=Y
  ...
  PASS Q06  Who are our top 5 buyers by total spend?
          2.8s | cols=Y | exec=Y | acc=Y | fmt=Y
  ...
  PASS Q08  In 2017, which state had the highest furniture sales?
          2.1s | cols=Y | exec=Y | acc=Y | fmt=Y
  FAIL Q09  What percentage of our order lines lose money? (retried)
          1.8s | cols=Y | exec=N | acc=N | fmt=N
        error: SandboxViolation: Unknown name: 'len'. Only False, None, True, df, np, pd, result are available.
